    ...
    PageControl.prototype = {
        ...
        viewPage: function (pIndex) {
            ...
            var //firstStr = this.index > 1 ? "<a href=\"###\" data-type=\"first\">首页</a>" : "",
                ...
                lastStr = "<span class='total'>共 " + this.pageTotal + " 页</span>",
                lastInput = "<span class='go-text'>到第<input class='text' type='text'>页</span><a class='btn-go' href='javascript:'>确定</a>"
                //lastStr = this.index < this.pageTotal ? "<a href=\"###\" data-type=\"last\">末页</a>" : "",
                str = [];
            for (; start < pIndex; start++) {
                str.push("<a href=\"###\">" + start + "</a>");
            }
            str.push("<span class=\"" + this.className + "\">" + pIndex + "</span>");
            for (pIndex++; pIndex <= end; pIndex++) {
                str.push("<a href=\"###\">" + pIndex + "</a>");
            }
            this.target.html(firstStr + prevStr + startStr + str.join("") + endStr + nextStr + lastStr + lastInput);
        }
    }
